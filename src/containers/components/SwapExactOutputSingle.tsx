...
export function SwapExactOutputSingle(props: AddLiquidityProps) {
  const { pool } = props;
  const [tokenIn, setTokenIn] = useState(0);
  const [amountOutInput, setAmountOutInput] = useState("");
  const [amountInMaximumInput, setAmountInMaximumInput] = useState("");
      ...
      <Card.Body>
        ...
        <InputGroup className="mb-3">
          ...
          <FormControl
            ...
            onChange={(e) => setAmountOutInput(e.target.value)}
            aria-label="Default"
        ...
        <InputGroup className="mb-3">
          ...
          <FormControl
            value={amountInMaximumInput}
            onChange={(e) => setAmountInMaximumInput(e.target.value)}
            aria-label="Default"
            aria-describedby="inputGroup-sizing-default"
          />
        </InputGroup>
          ...
          onClick={async () => {
            ...
            try {
              ...
              if (tokenIn === 0) {
                ...
                if (!allowance.gte(parseEther(amountInMaximumInput))) {
                  alert(
              ...
              const tx = await window.swapRouter
                ...
                .exactOutputSingle({
                  ...
                  amountOut: parseEther(amountOutInput),
                  amountInMaximum: parseEther(amountInMaximumInput),
